...
from student.models import Course, Session_Year, CustomUser, Student, Staff, Subject, Staff_Notification, Staff_leave, \
...
# paxi add subject ko thau ma view subject garna
...
def STAFF_SEND_NOTIFICATION(request):
    staff = Staff.objects.all()
    see_notification = Staff_Notification.objects.all().order_by('-id')[0:20]
...
def STAFF_FEEDBACK(request):
    ...
    feedback_history = Staff_Feedback.objects.all().order_by('-id')[0:20]
...
def STUDENT_FEEDBACK(request):
    ...
    feedback_history = Student_Feedback.objects.all().order_by('-id')[0:20]
